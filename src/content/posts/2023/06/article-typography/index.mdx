...
A lot of people suggest sticking with the default font size for your main article content. That’s a good place to start, but don’t feel shy about bumping it up a bit. Many of my favorite websites do, too.
...
One way of doing this is by applying margins or negative margins to some elements, depending on how the layout works. Shifting the text is the strategy I used in the figure below.
...
Typography is a discipline that requires both a design eye and careful attention to detail. Fortunately, setting large blocks of text isn’t an activity that rewards novelty, so we can cheat a bit.
