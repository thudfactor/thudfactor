...
2. Before you sneer at a web developer's work, check to see if you are actually speaking to that developer.[^2]
...
But just in case: it's _ok_ to just use HTML and CSS if you are not interested in JavaScript personally. And if you use it only sparingly and reluctantly, the web is better for it.
...
These purely human failings lead to a lot of bad code, caused by using the wrong tool, using the tool _incorrectly_, or not slowing down enough to make good decisions -- or learn how to use the tool. Anything that promises to take the developer substantially out of the loop certainly plays to all three. That certainly includes AI, but it also includes the rather lengthy list of "no code" web building solutions, of which Figma Sites is only the most recent example.

Of the three, "greed" is the worst; it encourages and rewards the other two. When you go into a company and see terrible JavaScript doing things much better left to other tools, don't think "ugh, JavaScript." Think: what have these poor people been having to put up with?

---

That's certainly not to say that languages don't deserve criticism. JavaScript certainly has its faults, and there are many places it should not be used, and purposes for which it is not suited. Absent context, using JavaScript is not bad.

Much the same thing can be said for frameworks; React is old. It served a purpose quite well in 2013, and it has not (perhaps) caught up with what CSS and HTML can do in 2025. But I'll take a well-documented, thoughtfully designed, and carefully maintained framework _every day_ over a bespoke vanilla JavaScript application that has little documentation, considerably less testing, and probably no internal consistency at all.
...
[^3]: Well, I write Typescript. Which I like considerably less.
